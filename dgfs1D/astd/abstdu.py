...
def main():
    ...
    limitPos = get_kernel(kernlimsmod, "limitPos", 'PPP')
    limitPos_Op = lambda d_u, d_uavg, d_ulim: \
        limitPos.prepared_call(grid_Nv, block, d_u.ptr,
            d_uavg.ptr, d_ulim.ptr)
    ...
    def limitBound(d_ucoeff_in, d_ucoeff_out):
        assert comm.size == 1, "Not implemented"
        #assert basis.basis_kind == 'nodal-sem-gll', "Not implemented"
        ...
        cuda.memcpy_dtod(d_ulx.ptr, d_ucoeff_in.ptr, d_ucoeff_in.nbytes)

        # limit functions in all cells
        limitPos_Op(d_ulx, d_uavg, d_ucoeff_out)
    ...
    while(time < tf):
        ...
        if limitOn: limitBound(d_ucoeff, d_ucoeff)
