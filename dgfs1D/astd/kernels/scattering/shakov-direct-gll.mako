    ...
    {   
        ...
        %for iter in [5, 6, 7, 8, 9, 10]:
            U${ar}[idx*${nalph}+${iter}] = ((${"+ ".join([
              ...
              + c0*dt*prefac*U${ar}[idx*${nalph}+0]*pow(T,${1-omega})
                *(
                    %if iter<=7:
                      0.5*U${ar}[idx*${nalph}+0]*T
                    + U${ar}[idx*${nalph}+${iter-4}]*U${ar}[idx*${nalph}+${iter-4}]
                    %else:
                      U${ar}[idx*${nalph}+${iter-7}]*U${ar}[idx*${nalph}+${(iter-7)%3+1}]
                    %endif

                      /U${ar}[idx*${nalph}+0]
                )
            )/(1+c0*dt*prefac*U${ar}[idx*${nalph}+0]*pow(T,${1.-omega}));
        %endfor

        // Evolve heat flux
        % for iter, (qq1, qq2, qq3) in enumerate(['035', '314', '542']):
            <% q1, q2, q3 = map(int, (qq1, qq2, qq3)) %>
            U${ar}[idx*${nalph}+${iter}+11] = ((${"+ ".join([
                        "-a{4}*U{1}[idx*{2}+{3}] + dt*b{4}*LU{1}[idx*{2}+{3}] + dt*c{5}*LM{1}[idx*{2}+{3}]".format(
                            ar, i, nalph, 11+iter, ar-i-1, ar-i) for i in range(ar)
                    ])})
                + c0*dt*prefac*U${ar}[idx*${nalph}+0]*pow(T,${1-omega})
                  *(
                    ${1.-Pr}*(
                      -2.*(
                          U${ar}[idx*${nalph}+5+${q1}]*U${ar}[idx*${nalph}+1]
                        + U${ar}[idx*${nalph}+5+${q2}]*U${ar}[idx*${nalph}+2]
                        + U${ar}[idx*${nalph}+5+${q3}]*U${ar}[idx*${nalph}+3]
                      )/U${ar}[idx*${nalph}+0]
                      + U${ar}[idx*${nalph}+1+${iter}]*(
                          U${ar}[idx*${nalph}+1]*U${ar}[idx*${nalph}+1]
                        + U${ar}[idx*${nalph}+2]*U${ar}[idx*${nalph}+2]
                        + U${ar}[idx*${nalph}+3]*U${ar}[idx*${nalph}+3]
                      )/pow(U${ar}[idx*${nalph}+0], 2.)
                      - 1.5*U${ar}[idx*${nalph}+1+${iter}]*T
                    )
                  + (
                      U${ar}[idx*${nalph}+1+${iter}]*(
                          U${ar}[idx*${nalph}+1]*U${ar}[idx*${nalph}+1]
                      ...
                      + 2.5*U${ar}[idx*${nalph}+1+${iter}]*T
                    )
                  )
                )/(
                    1 + ${Pr}*c0*dt*prefac*U${ar}[idx*${nalph}+0]*pow(T, ${1-omega})
                );                
        %endfor
    }
}
