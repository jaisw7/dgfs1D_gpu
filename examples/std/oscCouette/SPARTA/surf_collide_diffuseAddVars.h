...
SurfCollideStyle(diffuseAddVars,SurfCollideDiffuseAddVars)

#else

#ifndef SPARTA_SURF_COLLIDE_DIFFUSEADDVARS_H
#define SPARTA_SURF_COLLIDE_DIFFUSEADDVARS_H

#include "surf_collide.h"

namespace SPARTA_NS {

class SurfCollideDiffuseAddVars : public SurfCollide {
 public:
  SurfCollideDiffuseAddVars(class SPARTA *, int, char **);
  ~SurfCollideDiffuseAddVars();
  void init();
 ...
 private:
  ...
  char *vxstr;                // Vx variable name (NULL if constant)
  int vxvar;                  // index of equal-style variable

  char *vystr;                // Vy variable name (NULL if constant)
  int vyvar;                  // index of equal-style variable

  char *vzstr;                // Vz variable name (NULL if constant)
  int vzvar;                  // index of equal-style variable

  class RanPark *random;     // RNG for particle reflection

  void diffuseAddVars(Particle::OnePart *, double *);
};
